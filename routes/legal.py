...
def get_bail_score(prisoner: dict) -> int:
    score = 50
    offense_map = {
        "minor": 20, "bailable": 10,
        "non-bailable": -15, "heinous": -30
    }
    ...
    if prisoner.get("prior_record"):
        score -= 20
    if not prisoner.get("has_lawyer"):
        score += 8
...
async def get_all_undertrials(
    status: str = "all",
    district: str = ""
):
    """Get all undertrials with alert status, optional filter"""
    try:
        query = supabase.table("undertrials").select("*")

        if district:
            query = query.eq("district", district)

        result = query.execute()
        prisoners = result.data or []

        enriched = []
        for p in prisoners:
            days = days_in_custody(p["arrest_date"])
            alert = get_alert_status(days)
            bail_score = get_bail_score(p)
        ...
        # Sort: red first, then yellow, then green, then by days desc
...
async def get_districts():
    """Get list of all distinct districts"""
    try:
        result = supabase.table("undertrials") \
            .select("district") \
            .execute()

        districts = list(set(r["district"] for r in (result.data or []) if r.get("district")))
        districts.sort()
...
async def get_dashboard_stats():
    """Get summary stats for legal dashboard"""
    try:
        result = supabase.table("undertrials").select("*").execute()
        prisoners = result.data or []

        total = len(prisoners)
        overdue = sum(1 for p in prisoners if days_in_custody(p["arrest_date"]) > 90)
        approaching = sum(1 for p in prisoners
                         if 60 < days_in_custody(p["arrest_date"]) <= 90)
